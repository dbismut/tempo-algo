export default class TinySynth {
	masterVol = 0.5
	reverbLev = 0.3
	quality = 1
	debug = 0
  ...
  */
	...
	setQuality(q) {
		...
		for (let i = 0; i < this.drummap0.length; ++i) this.setTimbre(1, i + 35, this.drummap0[i])
		if (this.quality) {
			for (let i = 0; i < this.program1.length; ++i) this.setTimbre(0, i, this.program1[i])
	...
	setTimbre(m, n, p) {
		...
		function filldef(p) {
			for (n = 0; n < p.length; ++n) {
				for (let k in defp) {
					if (!p[n].hasOwnProperty(k) || typeof p[n][k] === 'undefined') p[n][k] = defp[k]
				}
			}
			return p
		}
		...
		for (let i = 0; i < p.length; ++i) {
			...
			switch (pn.w[0]) {
				case 'n':
					...
					if (pn.p !== 1) this._setParamTarget(o[i].playbackRate, (fp[i] / 440) * pn.p, t, pn.q)
	...
	setPan(ch, v, t) {
		if (this.chpan[ch]) this.chpan[ch].pan.setValueAtTime((v - 64) / 64, this._tsConv(t))
	}
	...
	setBend(ch, v, t) {
		...
		for (let i = this.notetab.length - 1; i >= 0; --i) {
			...
			if (nt.ch === ch) {
				for (let k = nt.o.length - 1; k >= 0; --k) {
					if (nt.o[k].frequency) nt.o[k].detune.setValueAtTime(this.bend[ch], t)
				}
			}
		}
	}
		...
		switch (cmd) {
			case 0xb0 /* ctl change */:
				switch (msg[1]) {
					...
					case 6 /* data entry msb */:
						if (this.rpnidx[ch] === 0) this.brange[ch] = (msg[2] << 7) + (this.brange[ch] & 0x7f)
						break
					case 38 /* data entry lsb */:
						if (this.rpnidx[ch] === 0) this.brange[ch] = (this.brange[ch] & 0x380) | msg[2]
						break
			...
			case 0xf0:
				if (msg[0] !== 254 && this.debug) {
					var ds = []
					for (let ii = 0; ii < msg.length; ++ii) ds.push(msg[ii].toString(16))
				}
				if (
					msg[1] === 0x41 &&
					msg[2] === 0x10 &&
